...
function main(option) {
  var comp = app.project.activeItem; // const audioFolderName = Folder.selectDialog(
  //   "please select destination audio folder").fsName

  var audioFolderName = "F:/code/aftereffects/test";
  ...
  try {
    for (_iterator.s(); !(_step = _iterator.n()).done;) {
      ...
      if (selectedLayer instanceof TextLayer) {
        var text = String(selectedLayer.sourceText.value);
        var audioPath = audioFolderName + "\\" + makeFileName(text) + ".wav";
        makeSpeech(text, audioPath, optionText);
        ...
        comp.layers.add(importAudio);
        importAudio.inPoint = selectedLayer.inPoint;
      }
    }
  } catch (err) {
    _iterator.e(err);
