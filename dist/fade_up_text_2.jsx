...
var DEFAULT_PARAMETERS = {
  fadeSpeed: 815, fadeInterval: 4.0, sequenceLayers: 0
};
var refComps;
...
function applyFadeToLayer(textLayer) {
  ...
  var selector = animator.property(
    "ADBE Text Selectors").addProperty("ADBE Text Selector");
  var opacity = animator.property(
    "ADBE Text Animator Properties").addProperty("ADBE Text Opacity");
  opacity.setValue(0);
...
function setReferenceComps(){
  ...
  if (!baseComp) {
    baseComp = app.project.items.addComp(
      "base comp", DEFAULT_COMP_SETTINGS.width, DEFAULT_COMP_SETTINGS.height,
      DEFAULT_COMP_SETTINGS.pixelAspect, DEFAULT_COMP_SETTINGS.duration,
      DEFAULT_COMP_SETTINGS.frameRate);
  }
  ...
  if (!textBGComp) {
    ...
    rectProperty.property("size").setValue(
      [DEFAULT_COMP_SETTINGS.width * .95, DEFAULT_COMP_SETTINGS.height / 4]);
    var fillProperty = shapeProperty.addProperty("ADBE Vector Graphic - Fill");
    fillProperty.property("Color").setValue([250, 250, 250, 255]);
    textArea.property("position").setValue(
      [DEFAULT_COMP_SETTINGS.width / 2, DEFAULT_COMP_SETTINGS.height * 5 / 6]);
  ...
  if (!textSetComp) {
    ...
    var speedControl = paramsLayer.property("ADBE Effect Parade").addProperty(
      "ADBE Slider Control");
    speedControl.name = "speed";
    speedControl.property("Slider").setValue(DEFAULT_PARAMETERS.fadeSpeed);
    var intervalControl = paramsLayer.property(
      "ADBE Effect Parade").addProperty("ADBE Slider Control");
    intervalControl.name = "interval";
    intervalControl.property("Slider").setValue(DEFAULT_PARAMETERS.fadeInterval);
    var sequenceLayersControl = paramsLayer.property(
      "ADBE Effect Parade").addProperty("ADBE Checkbox Control");
    sequenceLayersControl.name = "sequence layers";
    sequenceLayersControl.property("Checkbox").setValue(
      DEFAULT_PARAMETERS.sequenceLayers);
...
function setScriptComp(scriptComp, text) {
  ...
  var sequenceLayers = effects.property(
    "sequence layers").property("Checkbox").value;
  var currentTime = 0;
...
function main() {
  ...
  if (textFiles) {
    ...
    for (var i = 0; i < textFiles.length; i++) {
      ...
      var text = readTextFile(textFile).split(
