...
function createDialog(title, message) {
  ...
  dig.btn = dig.add('button',centerBounds(dig_bounds, 100, 150, 40), "OK!", { name:"ok"});
  dig.center();
  return dig;
}
...
function align(target_layer, metrix, vert, horiz) {
  ...
  target_layer.property("Anchor Point").setValue(
...
function selectAlignment() {
  ...
  vertDig.dropdown = vertDig.add('dropdownlist', centerBounds(dig_bounds, 50, 220, 30),
                         ['top', 'middle', 'bottom']);
  vertDig.dropdown.selection = 1;
  vertDig.show();

  var horDig = createDialog("Select horizontal alignments","Horizontal align by..");
  horDig.dropdown = horDig.add('dropdownlist', centerBounds(dig_bounds, 50, 220, 30),
                         ['left', 'center', 'right']);
